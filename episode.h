...
public:
  ...
  agent &take_turns(agent &play, agent &evil) {
    ep_time = millisec();
    return (std::max(step() + 1, 9ul) & 1) ? evil : play;
  }
  agent &last_turns(agent &play, agent &evil) { return take_turns(evil, play); }
...
public:
  ...
  time_t time(unsigned who = -1u) const {
    ...
    case action::place::type:
      if (ep_moves.size())
        time += ep_moves[0].time, i = 1;
      // no break;
    case action::slide::type:
      while (i < ep_moves.size())
        time += ep_moves[i].time, i += 2;
      break;
    default:
  ...
  std::vector<action> actions(unsigned who = -1u) const {
    ...
    case action::place::type:
      if (ep_moves.size())
        res.push_back(ep_moves[0]), i = 1;
      // no break;
    case action::slide::type:
      while (i < ep_moves.size())
        res.push_back(ep_moves[i]), i += 2;
      break;
    default:
...
protected:
  ...
  struct meta {
    std::string tag;
    time_t when;
    meta(const std::string &tag = "N/A", time_t when = 0)
        : tag(tag), when(when) {}
  ...
  static board initial_state() { return board(); }

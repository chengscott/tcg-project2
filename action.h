...
public:
  ...
  virtual std::istream &operator<<(std::istream &in) {
    ...
    for (auto proto = entries().begin(); proto != entries().end(); proto++) {
      if (proto->second->reinterpret(this) << in)
        return in;
      in.clear(state);
    }
    return in.ignore(2);
...
protected:
  ...
  virtual action &reinterpret(const action *a) const {
...
public:
  ...
  place(unsigned pos, unsigned tile)
      : action(place::type | (pos & 0x0f) | (std::min(tile, 35u) << 4)) {}
